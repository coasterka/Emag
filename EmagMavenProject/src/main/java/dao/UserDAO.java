...
public class UserDAO extends AbstractDAO implements IUserDAO {
	...
	private static final String UPDATE_USER_SQL = "UPDATE users SET name = ?, username = ?, password = ?, address = ? WHERE id = ?;";
	...
	public int addUser(User user) throws UserDAOException {
		if (user != null) {
			...
			try {
				PreparedStatement ps = getCon().prepareStatement(INSERT_NEW_USER_SQL,
	...
	public User getUserById(int userId) throws UserDAOException, EmagInvalidArgumentException{
		...
		try {
			PreparedStatement ps = getCon().prepareStatement(FIND_USER_BY_ID_SQL);
			ps.setInt(1, userId);
			...
			if (result.getBoolean(6) == true) {				
				...
				wantedUser = new Customer(name, username, password, address);
